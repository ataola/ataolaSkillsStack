...
function getUrlParam(sUrl, sKey) {
    ...
    paramsArr.forEach((val) => {
       ...
       if(!resObj[k]) {
           ...
           resObj[k] = [].concat(resObj[k], v);
       }
    });
    return !sKey ? resObj : resObj[sKey] || '';
}
```
运行时间：1674ms,占用内存：81396k

但我个人还是觉得这题不是很严谨就比如它跑的测试用例返回结果是[1,2,3]而这里['1','2','3']也是通过了，反正做的多了字符串转换过来数字特别要注意，就是提一下。
